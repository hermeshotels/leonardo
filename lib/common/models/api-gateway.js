...
module.exports = function (ApiGateway) {
  ...
  ApiGateway.recoverByCode = function (code, resParams, cb) {
    ApiGateway.app.models.BolReservation.findOne({
      where: {
        code: code
      }
      ...
      if (!data) {
        console.log('[RECOVER] Reservation not found: ' + data + ' with code: ' + code);
        ...
        console.log('[RECOVER] Trying to rebuild reservation: ' + data);
        ApiGateway.app.models.BolReservation.create(data, function (error, model) {
          if (error) return cb(error, null);
          recoverFromErmes(data.channel, data.rescodes, data.email).then(function (reslist) {
            if (reslist[0].indexOf('errore') > -1) {
              console.log('Reservation not found code: ' + data.rescodes + ', channel: ' + data.channel + ', email: ' + data.email);
      ...
      recoverFromErmes(data.channel, data.rescodes, data.email).then(function (reslist) {
        if (reslist[0].indexOf('errore') > -1) {
          _logger2.default.verbose('Reservation not found code: ' + data.rescodes + ', channel: ' + data.channel + ', email: ' + data.email);
          // errore nel recupero della prenotazione
          return cb(null, null);
        }
        _voucher2.default.format(reslist, data.code, function (error, voucher) {
  ...
  function recoverFromErmes(channel, codes, email) {
    ...
    codes.forEach(function (code) {
      ...
      _request2.default.get({
        ...
        if (error) {
          _logger2.default.verbose(error);
